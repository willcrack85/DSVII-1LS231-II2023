...
class noticia extends modeloCredencialesBD
{
    ...
    /*/ ########** METODO PARA LISTAR DATOS FILTRADOS EN TABLA **####### /*/
    ...
    /*/ ########** METODO PARA MOSTRAR PAGINACION DE NOTICIAS EN TABLA **####### /*/
    public function getPaginacionNoticias($inicio, $total)
    {
        try {
            $MySQL = "CALL sp_listar_noticia_paginada('" . $inicio . "','" . $total . "')";
            $consulta = $this->_DB->query($MySQL);
            if ($consulta) {
                $resultado = $consulta->fetch_all(MYSQLI_ASSOC);
                $consulta->free_result();
